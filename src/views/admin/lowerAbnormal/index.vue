<template>
  <div class="app-container calendar-list-container">
    <el-form ref="searchForm" class="mtb-20" :model="searchForm" label-width="120px">
      ...
      <el-row>
        <el-col :span="8">
        <el-input style="width: 200px;" placeholder="实际承运人" v-model="searchForm.broker" @keyup.enter.native="handleSearch"></el-input>
        </el-col>
        <el-col :span="8">
        <el-input style="width: 200px;" placeholder="匹配异常状态" v-model="searchForm.invoice_status" @keyup.enter.native="handleSearch"></el-input>
        </el-col>
    ...
    <el-table ref="multipleTable" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="数据加载中..." border fit
      ...
      <el-table-column align="center" label="发车单号" show-overflow-tooltip>
        <template slot-scope="scope">
        <span>{{scope.row.serial_id}} </span>
        </template>
      </el-table-column>
      <el-table-column align="center" sortable label="发车单时间" prop="create_time">
        <template slot-scope="scope">
          <span>
      ...
      <el-table-column align="center" label="实际承运人">
      ...
      <el-table-column align="center" label="流水单号">
        <template slot-scope="scope"> <span v-for="value in scope.row.lower_to_order"> {{ value }}&nbsp </span> </template>
      </el-table-column>
      <el-table-column align="center" label="匹配异常">
        <template slot-scope="scope">{{ scope.row.invoice_status | parseStateStatus() }}</template>
      </el-table-column>
      <el-table-column align="center" class-name="status-col" sortable prop="txid" label="上链ID">
        <template slot-scope="scope">
...
<script>
import { fetchOrderStartList } from '@/api/user'
