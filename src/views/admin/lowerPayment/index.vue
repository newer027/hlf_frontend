<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table ref="multipleTable" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="数据加载中..." border fit
      ...
      <el-table-column align="center" label="资金流水号" show-overflow-tooltip>
      ...
      <el-table-column align="center" label="签约主体">
        <template slot-scope="scope">{{ scope.row.invoice_org }}</template>
      </el-table-column>
      <el-table-column align="center" label="承运人">
        <template slot-scope="scope">{{ scope.row.broker }}</template>
      </el-table-column>
      <el-table-column align="center" label="流水金额">
        <template slot-scope="scope">{{ scope.row.amount }}</template>
      </el-table-column>
      <el-table-column align="center" label="发车单数量">
...
<script>
import { fetchLowerPaymentList } from '@/api/user'
...
export default {
  ...
  methods: {
    getUserList() {
      ...
      fetchLowerPaymentList(this.searchForm).then(res => {
    ...
    reset() {
      ...
      this.searchForm.broker = this.searchForm.serial_id = this.searchForm.invoice_org = this.searchForm.min_create = this.searchForm.max_create = ''
      this.getUserList()
    },
    goWaybillDetail(id) {
      this.$router.push({ path: `lowerpayment/${id}` })
    },
    handleSearch() { // 搜索
